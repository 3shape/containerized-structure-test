...
if ($IsWindows) {
  ...
  docker run --name containertest-test --rm -v "${PWD}:C:/configs" -v //var/run/docker.sock:/var/run/docker.sock containertest test -c /configs/tests/gc-windows-config.yml -i mcr.microsoft.com/windows/nanoserver:1809
} else {
